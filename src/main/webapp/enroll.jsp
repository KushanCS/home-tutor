...
<head>
    ...
    <style>
        ...
        .enrollment-card { max-width: 600px; margin: 2rem auto; border-radius: 10px; box-shadow: 0 4px 15px rgba(0,0,0,0.1); }
        .course-header { background: linear-gradient(135deg, var(--primary-color), var(--secondary-color)); color: white; padding: 1.5rem; border-radius: 10px 10px 0 0; }
        .price-tag { color: var(--primary-color); font-weight: bold; font-size: 1.5rem; }
        .btn-enroll { background: var(--primary-color); color: white; border: none; padding: 0.5rem 1.5rem; }
...
<nav class="navbar navbar-expand-lg navbar-light bg-white shadow-sm">
    <div class="container">
        <a class="navbar-brand" href="home-page.jsp"><i class="fas fa-graduation-cap me-2"></i>MetaTutor</a>
        <button class="navbar-toggler" data-bs-toggle="collapse" data-bs-target="#navbarNav"><span class="navbar-toggler-icon"></span></button>
        <div class="collapse navbar-collapse" id="navbarNav">
            <ul class="navbar-nav me-auto"><li class="nav-item"><a class="nav-link" href="student-course.jsp">My Courses</a></li></ul>
            <div class="d-flex">
...
<div class="card enrollment-card">
    ...
    <div class="card-body p-4">
        ...
        <p class="price-tag"><strong>Price:</strong> $<%= String.format("%.2f", selected.getPrice()) %></p>
        <p><strong>Duration:</strong> <%= selected.getDuration() %></p>
        <div class="text-center mt-4">
            <a href="paynow.jsp?name=<%= selected.getName() %>" class="btn btn-enroll me-2">
                <i class="fas fa-credit-card me-1"></i>Pay Now
            </a>
        </div>
        <% } else { %>
        <p class="text-danger text-center">Sorry, that course was not found.</p>
        <% } %>
    </div>
</div>
